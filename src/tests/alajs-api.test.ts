...
import { Api } from '../alajs-api';
import { JsonRpc } from '../alajs-jsonrpc';
import { JsSignatureProvider } from '../alajs-jssig';
...
describe('alajs-api', () => {
    ...
    it('getAbi returns an abi', async () => {
        const response = await api.getAbi('testalatoken');
    ...
    it('getContract returns a contract', async () => {
        const response = await api.getContract('testalatoken');
